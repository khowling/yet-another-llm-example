...
# Deploy infra...
DEPLOY_OUTPUT=$(az deployment group create -g $rgName  --template-file ./setup/azure/infra/main.bicep  --parameters uniqueName=${uniqueName} localDeveloperId=${localDeveloperId} --query [properties.outputs.cosmosConnectionURL.value,properties.outputs.storageAccountName.value,properties.outputs.openAIEndpoint.value,properties.outputs.openAIModel.value,properties.outputs.acrName.value,properties.outputs.acaName.value] -o tsv)
